...
# コーヒーの種類を選ばせる
coffee_choice = input(
    "what kind of coffee do you want?latte or cappuccino or espresso :")


# 材料足りるかを判断する
def ingredient_compare(coffee_choice):
    # 現在の材料の在庫状況をプリントする
    print(f"the report of ingredient : coffee is {ingredient['coffee']}, suger is {
          ingredient['suger']}, milk is {ingredient['milk']}")
    # コーヒーの値段をプリントする
    print(f"the price of your order is {
          coffee_ingredient[coffee_choice]["price"]}")
...
def buy_coffee(coffee_choice):
    if ingredient_compare(coffee_choice) and coin_compare(coffee_choice):
        ...
        print(
            f"It is {coffee_ingredient[coffee_choice]['price']},You're short on money.")
